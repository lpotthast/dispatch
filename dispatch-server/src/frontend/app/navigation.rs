...
use crate::frontend::routes::{routes, with_project};
use crate::{
    frontend::{automation::service::automation_service, projects::service::project_service},
...
use crate::frontend::components::selected_project_signal;
...
pub(crate) fn TopBar() -> impl IntoView + 'static {
    ...
    let initial = store.cached_page_untracked();
    ...
    let result = query_resource(
        initial,
        || (),
        move |()| store_for_cache.cached_page(),
        move |()| {
            let store = store.clone();
            async move { store.load_page().await }
        },
        move |(), value| store_for_seed.seed_page(value),
        move |()| store_for_refresh.invalidate_page(),
    );
    ...
    let projects = crate::frontend::projects::store::project_store().projects();
    ...
    let board_href = Memo::new(move |_| {
        with_project(
            routes::Root.materialize(),
            selected_project.get().as_deref(),
        )
    });
    let project_href = Memo::new(move |_| {
        with_project(
            routes::Project.materialize(),
            selected_project.get().as_deref(),
        )
    });
    let knowledge_href = Memo::new(move |_| {
        with_project(
            routes::Knowledge.materialize(),
            selected_project.get().as_deref(),
        )
    });
    let triggers_href = Memo::new(move |_| {
        with_project(
            routes::Automation.materialize(),
            selected_project.get().as_deref(),
        )
    });
    let runs_href = Memo::new(move |_| {
        with_project(
            routes::Runs.materialize(),
            selected_project.get().as_deref(),
        )
    });
    let system_href = Memo::new(move |_| {
        with_project(
            routes::System.materialize(),
            selected_project.get().as_deref(),
        )
    });
    let metrics_href = Memo::new(move |_| {
        with_project(
            routes::Metrics.materialize(),
            selected_project.get().as_deref(),
        )
    });
    let projects_href = Memo::new(move |_| {
        with_project(
            routes::Projects.materialize(),
            selected_project.get().as_deref(),
        )
    });
    let api_href = Memo::new(move |_| {
        with_project(
            routes::ApiDocs.materialize(),
            selected_project.get().as_deref(),
        )
    });
    let codex = crate::frontend::codex::store::codex_store();
...
fn TopBarAutomationControl(
    ...
    let toggle_running_action = Action::new(move |_: &()| {
    ...
    let pending = toggle_running_action.pending();
    let toggle_running = move |_| {
        if pending.get_untracked() {
            return;
        }
        toggle_running_action.dispatch(());
    };
...
fn TopBarAutoCommitControl(
    ...
    let update_action = Action::new(move |input: &bool| {
        let next = *input;
        ...
        let service = service.clone();
        async move {
            if service.update_auto_commit(project, next).await.is_err() {
...
fn project_selection_href(active: ActivePage, project: &str) -> String {
    let path = match active {
        ActivePage::Board => routes::Root.materialize(),
        ActivePage::Project => routes::Project.materialize(),
        ActivePage::Knowledge => routes::Knowledge.materialize(),
        ActivePage::Triggers => routes::Automation.materialize(),
        ActivePage::Runs => routes::Runs.materialize(),
        ActivePage::System => routes::System.materialize(),
        ActivePage::Metrics => routes::Metrics.materialize(),
        ActivePage::Projects => routes::Projects.materialize(),
        ActivePage::Api => routes::ApiDocs.materialize(),
    };
    with_project(path, Some(project))
}
