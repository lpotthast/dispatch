...
use crate::{
    frontend::components::selected_project_signal,
    shared::view_models::{
...
pub fn PageMetrics() -> impl IntoView {
    ...
    let store = crate::frontend::metrics::store::metrics_store();
    ...
    let result = query_resource(
        ...
        move |project| store_for_cache.cached_page(project),
        move |project| {
            let store = store_for_load.clone();
            let project = project.clone();
            async move { store.load_page(project).await }
        },
