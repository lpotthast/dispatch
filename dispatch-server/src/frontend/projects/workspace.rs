...
use dispatch_types::WorkspaceBarData;
use leptos::prelude::*;
...
pub(crate) fn WorkspaceBar() -> impl IntoView {
    ...
    let initial = store.cached_workspace_bar_untracked(&selected_project.get_untracked());
    ...
    let result = query_resource(
        ...
        move |selected_project| store_for_cache.cached_workspace_bar(selected_project),
        move |selected_project| {
            let store = store_for_load.clone();
            let selected_project = selected_project.clone();
            async move { store.load_workspace_bar(selected_project).await }
        },
        move |key, value| store_for_seed.seed_workspace_bar(key, value),
        move |key| store_for_refresh.invalidate_workspace_bar(key),
    );
    view! {
...
fn WorkspaceOpenButton(
    ...
    let open_action = Action::new(move |_: &()| {
    ...
    let pending = open_action.pending();
    let open = move |_| {
        if pending.get_untracked() || disabled {
            return;
        }
        open_action.dispatch(());
    };
