use crate::frontend::queries::{query_resource, seeded_query_resource};
use crate::frontend::{items::types::ItemDetail, runs::types::RunDetail};
use {
    crate::frontend::board::types::BoardShell,
    dispatch_types::{BoardItemView, BoardItemsSection, BoardRunPreview},
};
mod view;
...
use crate::{
    frontend::{
        ...
        items::{
            components::{encode_path, provide_work_item_states_signal},
            creation::{
...
impl BoardDrawerData {
    fn identity(&self) -> (i64, i64, Option<i64>) {
        match self {
            Self::Item(page) => (page.item.project_id, page.item.id, None),
            Self::Run { item, run } => {
                (item.item.project_id, item.item.id, Some(run.run_log.run.id))
            }
        }
    }
    fn item(&self) -> ItemDetail {
        match self {
            Self::Item(page) => page.clone(),
            Self::Run { item, .. } => item.clone(),
        }
    }
}
...
pub fn PageBoard() -> impl IntoView {
    ...
    let store = crate::frontend::board::store::board_store();
        ...
        >
            <main class="page-shell">
                ...
                <For
                    ...
                    key=|page| page.selected_project_view.as_ref().map(|project| project.id)
...
#[component]
fn BoardContent(page: Signal<BoardShell>) -> impl IntoView {
    ...
    if let (Some(project), Some(project_view)) = (selected_project.clone(), selected_project_view) {
        ...
        let store = crate::frontend::board::store::board_store();
        let initial_section = store.cached_items_untracked(&project);
        let project_for_loader = project.clone();
        let store_for_cache = store.clone();
        let store_for_load = store.clone();
        let store_for_refresh = store.clone();
        let store_for_seed = store.clone();
        let section = seeded_query_resource(
            initial_section,
            move || project_for_loader.clone(),
            move |project| store_for_cache.cached_items(project),
            move |project| {
                let store = store_for_load.clone();
                let project = project.clone();
                async move { store.load_items(project).await }
            },
            move |key, value| store_for_seed.seed_items(key, value),
            move |key| store_for_refresh.invalidate_items(key),
        );
        ...
        let create_item_label_suggestions =
            Signal::derive(move || page.with(|page| page.label_suggestions.clone()));
...
fn BoardInspectorDrawer(
        ...
        async move {
            let result = async move {
                ...
                match selection.run_id {
                    Some(run_id) => {
                        let run = run_store.load_log(Some(project), Some(run_id)).await?;
    ...
    #[cfg(not(feature = "ssr"))]
    {
        ...
        Effect::new(move |_| {
            let failure = drawer_data
                .map(|(loaded, result)| {
                    (*loaded == selection.get_untracked())
                        .then(|| result.as_ref().err().map(ToString::to_string))
                        .flatten()
                })
                .flatten();
    ...
    let drawer_content = Signal::derive(move || {
        drawer_data.get().and_then(|(loaded, result)| {
            (loaded == selection.get())
        ...
        >
            ...
            >
                <Transition fallback=move || view! {
                    ...
                    <For each=move || drawer_content.get() key=BoardDrawerData::identity children=move |initial| {
                        let identity = initial.identity();
                        let data = Signal::derive(move || drawer_content.get().filter(|data| data.identity() == identity).unwrap_or_else(|| initial.clone()));
                        view! { <BoardDrawerContent project=project.clone() data controls=BoardDrawerControls {
                            set_item_editor_context, attempt_action, open_drawer, close_drawer, on_item_editor_return, refresh: drawer_refresh,
                        }/> }
                    }/>
                </Transition>
            </div>
...
#[component]
fn BoardDrawerContent(
    project: String,
    data: Signal<BoardDrawerData>,
    ...
    match data.get_untracked() {
        BoardDrawerData::Item(page) => {
            let item_id = page.item.id;
            let title = move || format!("#{} {}", item_id, data.get().item().item.title);
            let full_href =
                crate::frontend::routes::routes::Item.materialize(encode_path(&project), item_id);
            ...
            let item_detail = view! {
                <ItemDetailContent
                    page=Signal::derive(move || data.get().item())
                    refresh
                    interactive
        ...
        BoardDrawerData::Run { item, run } => {
            ...
            let run_for_detail = Signal::derive(move || match data.get() {
                BoardDrawerData::Run { run, .. } => *run,
                _ => (*run).clone(),
            });
            view! {
                ...
                <div class="board-drawer-body run-log">
                    <RunLogContent page=run_for_detail show_thinking_history=show_thinking_history.into() toggle_thinking_history/>
                </div>
            }
            .into_any()
        }
