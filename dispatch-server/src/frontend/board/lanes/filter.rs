...
pub(crate) fn condition_field_renderer<F: TypeErasedField>() -> FieldRenderer<F> {
    FieldRenderer::new(
...
pub(crate) fn swim_lane_filter_field_renderer<F: TypeErasedField>() -> FieldRenderer<F> {
    condition_field_renderer()
}

pub(crate) fn condition_editor_view(
    raw: String,
    disabled: bool,
    ...
    fn supported_clause_value_modes_round_trip() {
        let draft = FilterGroupDraft {
            ...
            elements: vec![
                ...
                clause("needs-verification", FilterClauseMode::Present),
                clause("dispatch:automation-blocked", FilterClauseMode::Absent),
