...
impl Jobs {
    pub(super) fn new(project: StoredValue<String>, workspace_refresh: RwSignal<u64>) -> Self {
        ...
        let store = StoredValue::new(crate::frontend::knowledge::store::knowledge_store());
        let result = LocalResource::new(move || {
            let revision = jobs.refresh.get();
            let store = store.get_value();
            let project = project.get_value();
            if revision > 0 {
                store.invalidate_jobs(&project);
            }
            async move { store.list_jobs(project).await }
        });
        Effect::new(move || {
            let result = result.get();
            let current = store.get_value().cached_jobs(&project.get_value());
            if let Some(result) = current.map(Ok).or(result) {
                match result {
                    Ok(list) => {
...
pub(super) fn JobsDrawer(
    ...
    let resolve_owner = Action::new(move |id: &String| {
    ...
    let settings = LocalResource::new(move || {
        let project = project.get_value();
        async move { store.get_value().job_settings(project).await }
    });
    Effect::new(move || {
    ...
    let save_application = Action::new(move |mode: &ApplicationMode| {
        let mode = *mode;
        ...
        async move {
            match service
                ...
                .await
            {
                Ok(settings) => application.set(settings.application_mode),
                Err(e) => jobs.error.set(Some(error_message(e))),
            }
        }
    });
    let save_mode = move |event| {
        ...
        save_application.dispatch(mode);
    };
    let mutate_job = Action::new(move |(id, action): &(i64, JobAction)| {
        let id = *id;
    ...
    let start_job = Action::new(move |request: &StartKnowledgeJob| {
    ...
    let pending = Signal::derive(move || mutate_job.pending().get() || start_job.pending().get());
    let action = Callback::new(move |input: (i64, JobAction)| {
        if !pending.get_untracked() {
            mutate_job.dispatch(input);
        }
    });
    let start = move |_| {
        if pending.get_untracked() {
        ...
        start_job.dispatch(StartKnowledgeJob {
    ...
    let detail = LocalResource::new(move || {
        ...
        async move {
            match id {
                Some(id) => store.job_detail(project, id).await.map(Some),
                None => Ok(None),
            }
        }
    });
    view! {<div class="knowledge-jobs">
        <label>"Changes to accepted documents"<select aria-label="Knowledge application mode" disabled=move || save_application.pending().get() prop:value=move ||if application.get()==ApplicationMode::Automatic{"automatic"}else{"review"} on:change=save_mode><option value="review">"Review proposals"</option><option value="automatic">"Apply validated changes automatically"</option></select></label>
        ...
        {move ||detail.get().map(|result|match result {
            Ok(Some(detail))=>view!{<JobDetail detail project=project.get_value() pending action select owner_select jobs/>}.into_any(),
...
fn JobDetail(
    ...
    let coverage = LocalResource::new(move || {
        ...
        let store = crate::frontend::knowledge::store::knowledge_store();
        let project = project.get_value();
        async move {
            store
                .job_coverage(project, id, (!aspect.is_empty()).then_some(aspect))
                .await
        }
    });
