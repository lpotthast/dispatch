...
use crate::frontend::{
    ...
    knowledge::service::{KnowledgeUiService, knowledge_ui_service},
};
use crudkit_leptos::{
...
fn KnowledgeWorkspace(project: String) -> impl IntoView {
    ...
    let jobs = jobs::Jobs::new(project, refresh);
    ...
    let graph_result = LocalResource::new(move || {
        let revision = refresh.get();
        let store = store.get_value();
        let project = project.get_value();
        if revision > 0 {
            store.invalidate_graph(&project);
        }
        async move { store.graph(project).await }
    });
    Effect::new(move || {
    ...
    let document_result = LocalResource::new(move || {
        ...
        let project = project.get_value();
        if revision > 0 {
            store.invalidate_document(&project, &path);
        }
        async move {
            if path.is_empty() {
                Ok(None)
            } else {
                store.document(project, path).await.map(Some)
            }
        }
    });
    Effect::new(move || {
    ...
    let save_document = Action::new(move |request: &KnowledgeSaveRequest| {
    ...
    Effect::new(move |_| editor.saving.set(save_document.pending().get()));
    let save = Callback::new(move |()| {
        if !save_document.pending().get_untracked() {
            save_document.dispatch(KnowledgeSaveRequest {
