...
impl KnowledgeJobsUiService {
    pub(crate) fn production(http: crate::frontend::http::HttpService) -> Self {
        Self {
            settings: http.request(|p| Box::pin(read_job_settings(p))),
            save_settings: http.request(|(p, s)| Box::pin(write_job_settings(p, s))),
            list: http.request(|p| Box::pin(list_jobs(p))),
            start: http.request(|(p, r)| Box::pin(start_job(p, r))),
            detail: http.request(|(p, id)| Box::pin(job_detail(p, id))),
            action: http.request(|(p, id, a)| Box::pin(mutate_knowledge_job(p, id, a))),
            coverage: http.request(|(p, id, a)| Box::pin(job_coverage(p, id, a))),
        }
    }
