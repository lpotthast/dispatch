use crate::frontend::runs::types::{CommandPresentation, RunOutputEntry};
#[cfg(test)]
mod tests;
...
pub(crate) fn RunOutput(
    #[prop(into)] output: Signal<Vec<AgentRunOutputPiece>>,
    #[prop(into)] active: Signal<bool>,
    #[prop(into)] show_thinking_history: Signal<bool>,
    toggle_thinking_history: Callback<()>,
) -> AnyView {
    let current_reasoning =
        Memo::new(move |_| current_reasoning_sequence(&output.get(), active.get()));
    let entries = Memo::new(move |_| compact_run_output(output.get()));
    let historical_count = Memo::new(move |_| {
        entries.with(|entries| {
            entries
                .iter()
                .filter(|entry| {
                    entry.piece.kind == AgentRunOutputKind::Reasoning
                        && Some(entry.piece.sequence) != current_reasoning.get()
                })
                .count()
        })
    });
    let visible = Memo::new(move |_| {
        entries
            ...
            .filter(|entry| {
                entry.piece.kind != AgentRunOutputKind::Reasoning
                    || Some(entry.piece.sequence) == current_reasoning.get()
                    || show_thinking_history.get()
            })
            .map(|entry| {
                let current = Some(entry.piece.sequence) == current_reasoning.get();
                (entry, current)
            })
            .collect::<Vec<_>>()
    });
    let toggle_title = move || {
        if show_thinking_history.get() {
    ...
    view! {
        <div class="run-output-controls">
            <Show when=move || { historical_count.get() > 0 }>
                <label class="thinking-history-toggle" title=toggle_title>
                    <Toggle state=show_thinking_history set_state=Callback::new(move |_: bool| toggle_thinking_history.run(())) size=ToggleSize::Small attr:aria-label=toggle_title/>
                    <span>{move || format!("Thinking ({})", historical_count.get())}</span>
                </label>
            </Show>
        </div>
        <div class="run-output">
            <Show when=move || visible.get().is_empty()><p class="muted run-output-empty">{move || if entries.get().is_empty() { "No output has been written yet." } else { "No visible output yet." }}</p></Show>
            <For each=move || visible.get() key=|(entry, current)| (entry.piece.sequence, *current) children=|(entry, current)| view! { <OutputEntry entry current/> }/>
        </div>
    }.into_any()
}

#[component]
fn OutputEntry(entry: RunOutputEntry, current: bool) -> impl IntoView {
    run_output_entry_view(entry, current)
}
