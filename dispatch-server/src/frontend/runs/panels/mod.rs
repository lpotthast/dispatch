...
use crate::frontend::{items::components::encode_path, runs::output::RunOutput};
use crate::{
    frontend::{
        automation::service::automation_service,
        codex::page::format_number,
        live_events::{
...
pub(crate) fn LiveRunsSection(project: String) -> impl IntoView + 'static {
    ...
    let initial = store.cached_section_untracked(&project);
    ...
    let section = query_resource(
        initial,
        move || project_for_loader.clone(),
        move |project| store_for_cache.cached_section(project),
        move |project| {
            let store = store_for_load.clone();
            let project = project.clone();
            async move { store.load_section(project).await }
        },
        move |key, value| store_for_seed.seed_section(key, value),
        move |key| store_for_refresh.invalidate_section(key),
    );
...
pub(crate) fn TriggerRunsPanel(
    ...
    let store_for_refresh = store.clone();
    let store_for_seed = store.clone();
    let trigger_runs = query_resource(
        initial,
        ...
        move |(project, trigger_id)| {
            ...
            async move {
                match trigger_id {
                    Some(trigger_id) => {
                        store.load_trigger_runs(project, trigger_id).await.map(Some)
                    }
                    None => Ok(None),
                }
            }
        ...
        move |(project, id), value| {
            if let (Some(id), Some(value)) = (id, value) {
                store_for_seed.seed_trigger_runs(project, id, value);
            }
        },
        move |(project, id)| {
            if let Some(id) = id {
                store_for_refresh.invalidate_trigger_runs(project, id);
            }
        },
    );
    let project_for_events = project.clone();
...
fn QueueAutomationEvaluation(
    ...
    let queue_action = Action::new(move |_: &()| {
    ...
    let pending = queue_action.pending();
    let queue = move |_| {
        if pending.get_untracked() {
            return;
        }
        queue_action.dispatch(());
    };
...
fn run_session_detail(
    ...
    let href =
        crate::frontend::routes::routes::RunLog.materialize(encode_path(project), detail.run.id);
    let model = detail
        .run
        .agent_model
        .clone()
