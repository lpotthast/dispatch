use crate::frontend::{board::lanes::filter::condition_editor_view, crudkit::*};
use dispatch_types::{
...
pub(crate) fn produced_work_field_renderer<F: TypeErasedField>() -> FieldRenderer<F> {
    FieldRenderer::new(
...
pub(crate) fn postconditions_field_renderer<F: TypeErasedField>() -> FieldRenderer<F> {
    FieldRenderer::new(
        move |_signals, _field: F, field_mode, field_options, value, value_changed| {
            let current = Signal::derive(move || json_string_from_value(&value.value.get()));
            match field_mode {
