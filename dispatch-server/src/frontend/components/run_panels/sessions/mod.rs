...
pub(super) fn RunSessionsPanel(
    ...
    let initial_detail = match selected_run_id.get_untracked() {
        Some(run_id) => service.cached_detail_untracked(&project, run_id).map(Some),
        None => Some(None),
    };
    let project_for_detail_input = project.clone();
    let service_for_detail_cache = service.clone();
    let service_for_detail_load = service.clone();
    let detail = cached_query(
        initial_detail,
        move || (project_for_detail_input.clone(), selected_run_id.get()),
        move |(project, run_id)| match run_id {
            Some(run_id) => service_for_detail_cache
                .cached_detail(project, *run_id)
                .map(Some),
            None => Some(None),
        },
        move |(project, run_id)| {
            let service = service_for_detail_load.clone();
            async move {
                match run_id {
                    Some(run_id) => service.load_detail(project, run_id).await.map(Some),
                    None => Ok(None),
                }
            }
        },
