...
use crate::{
    backend::{
        ...
        process_sessions::ProcessSessionRegistry, project_deletion::ProjectDeletionService,
        storage::Store,
    },
...
pub(crate) fn app_state() -> AppState {
    APP_STATE
        .read()
        .expect("Dispatch app state lock is poisoned")
        .clone()
        .expect("Dispatch app state must be installed before rendering")
}

pub(crate) fn install_app_state(state: AppState) {
    *APP_STATE
        .write()
        .expect("Dispatch app state lock is poisoned") = Some(state);
}
