use super::super::model::ProjectChangeSource;
use rootcause::{Result, prelude::*};
use sea_orm::{ColumnTrait, ConnectionTrait, EntityTrait, QueryFilter, QueryOrder};
use serde::{Deserialize, Serialize};

use crate::{
    backend::{entities::work_item_event, items::events::repository as work_item_events},
...
) -> Result<dispatch_types::WorkItemEventView>
where
    C: ConnectionTrait,
{
...
pub(super) async fn latest_system_prompt_event<C>(
    conn: &C,
    project_id: i64,
) -> Result<Option<dispatch_types::WorkItemEventView>>
where
    C: ConnectionTrait,
{
    Ok(
...
pub(super) fn system_prompt_event_to_view(
    project_name: &str,
    event: dispatch_types::WorkItemEventView,
    ...
    ProjectSystemPromptEventView {
        ...
        actor_type: event.actor_type.map(|actor| actor.as_storage().to_owned()),
    ...
    C: ConnectionTrait,
{
    work_item_events::record_event_with_attribution_in_tx(
        ...
        event_type,
        body,
        crate::backend::items::events::model::EventAttribution {
...
async fn list_project_text_events<C>(
    ...
    project_id: i64,
    event_type: WorkItemEventType,
) -> Result<Vec<dispatch_types::WorkItemEventView>>
where
    C: ConnectionTrait,
{
