...
use crate::{
    ...
    shared::view_models::{
        BoardWorkItemView, STATE_LABEL_KEY, WorkItemEventType, WorkItemEventView, WorkItemView,
    },
};
...
pub async fn list_items(
    ...
    list_items_for_project_id(store, project_id, state).await
}

pub(crate) async fn list_items_for_project_id(
    store: &Store,
    project_id: i64,
    state: Option<String>,
) -> Result<Vec<WorkItemView>> {
...
pub(crate) async fn list_board_items_for_project_id(
    store: &Store,
    project_id: i64,
) -> Result<Vec<BoardWorkItemView>> {
    crate::backend::metrics::time_repository("work_items.list_board", async {
        let items =
            work_item_views::BoardWorkItemModel::find_by_statement(Statement::from_sql_and_values(
        ...
        work_item_views::board_models_to_views(store, project_id, items).await
    })
    .await
}
...
pub async fn count_items_outside_work_item_states(
    ...
    let project_id = projects::project_id(store, project_name).await?;
    count_items_outside_work_item_states_for_project_id(store, project_id).await
}

pub(crate) async fn count_items_outside_work_item_states_for_project_id(
    store: &Store,
    project_id: i64,
) -> Result<i64> {
    let row = store
