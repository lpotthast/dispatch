...
impl CommitRuntime {
    pub(crate) fn capture_commit_baseline(&self, path: &Path, required: bool) -> CommitBaseline {
        CommitBaseline {
            required,
...
mod tests {
    ...
    fn commit_all(repo: &Repository, message: &str) -> String {
        ...
        let signature = git2::Signature::now("Dispatch Test", "dispatch@example.com").unwrap();
    ...
    fn commit_outcome_skips_when_workspace_is_not_git_repo() {
        ...
        let baseline = CommitRuntime.capture_commit_baseline(temp.path(), true);
    ...
    fn commit_outcome_allows_uncommitted_changes_when_commit_is_not_required() {
        ...
        fs::write(temp.path().join("README.md"), "initial\n").unwrap();
        commit_all(&repo, "Initial commit");
        let baseline = CommitRuntime.capture_commit_baseline(temp.path(), false);
        fs::write(temp.path().join("README.md"), "initial\nchanged\n").unwrap();
    ...
    fn read_only_commit_outcome_is_not_required_without_git_validation() {
        ...
        let evaluation = CommitRuntime.evaluate_commit_outcome_for_run(
            temp.path(),
            &baseline,
