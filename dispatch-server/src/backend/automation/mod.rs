pub(crate) mod knowledge_execution;
        ...
        if let Some(piece) = thread_event_output_piece(&event) {
                ...
                .is_some_and(|env| env.contains_key("DISPATCH_KNOWLEDGE_JOB_ID"))
            {
                ...
                let logs = start
                    .environment
                    .as_ref()
                    .and_then(|env| env.get("DISPATCH_KNOWLEDGE_LOG_DIR"))
                    .map(PathBuf::from)
                    .unwrap_or_else(automation_log_dir);
...
> {
    ...
    if let (Some(path), Some(pid)) = (
        env.get("DISPATCH_KNOWLEDGE_PROCESS_RECORD"),
        app_server.process_id(),
    ) {
        crate::backend::knowledge::jobs::runtime::processes::record(Path::new(path), pid)
            .map_err(CodexStreamStartError::Spawn)?;
    }
...
fn thread_options_with_raw_effort(options: &ThreadOptions, effort: &str) -> ThreadOptions {
