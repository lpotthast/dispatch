            ...
            loop {
                tokio::select! {
                    ...
                    _ = automation_interval.tick() => {
                        let cancellations = self.supervisor.project_cancellations().await;
                        if !cancellations.is_empty() {
                            let ids = cancellations.keys().copied().collect::<Vec<_>>();
                            if let Err(error) = self.scheduler.run_due(Some(&ids), Some(&cancellations)).await {
                                tracing::error!(error = %format_args!("{error:#}"), "automation trigger scheduler failed");
                            }
                        }
                    }
                    _ = maintenance_interval.tick() => {
                        if let Err(error) = self.claims.recover_all_configured().await {
                            tracing::error!(error = %format_args!("{error:#}"), "stale claim recovery failed");
                        }
                    }
                }
            }
