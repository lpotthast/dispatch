use crate::backend::automation::scheduling::{
    model::WorkItemAutomationCandidate,
    policy::{PRIORITY_SCORE_SECONDS, fairness_score as work_item_automation_score},
};
use crate::backend::projects::repository::ProjectRepository;
use assertr::prelude::*;
use crudkit_core::condition::Condition;
use dispatch_types::{AutomationRuleInput, RevisionChangeOperation};
...
use super::{configuration::*, model::*, repository::encoding::*};
use crate::backend::{
    entities::automation_trigger::{AutomationTrigger, AutomationTriggerActiveModel},
    storage::Store,
};
use dispatch_types::{
...
use crate::backend::{
    items::CreateWorkItem, items::labels::conditions as label_conditions, projects::CreateProject,
};
...
async fn produced_work_applies_fields_and_always_creates() {
    ...
    let first = crate::backend::automation::production::tests::service(&store, event_bus.clone())
        .produce("demo", &trigger)
        .await
        .unwrap();
    let second = crate::backend::automation::production::tests::service(&store, event_bus.clone())
        .produce("demo", &trigger)
        .await
        .unwrap();
...
async fn produced_work_deduplicates_by_trigger_until_completion() {
    ...
    let first = crate::backend::automation::production::tests::service(&store, event_bus.clone())
        ...
        .unwrap();
    let duplicate =
        crate::backend::automation::production::tests::service(&store, event_bus.clone())
            .produce("demo", &trigger)
            .await
            .unwrap();
    ...
    crate::backend::items::claims::tests::service(&store, event_bus.clone())
        .claim_specific_item("demo", first.id, "agent-test")
        .await
        .unwrap()
        .unwrap();
    crate::backend::items::claims::tests::service(&store, event_bus.clone())
    ...
    let evaluations = crate::backend::automation::revisions::service::service(&store)
        .evaluations("demo", Some(trigger.id), 20)
        .await
        .unwrap();
...
async fn produced_work_key_deduplication_spans_triggers_and_is_concurrency_safe() {
    ...
    let (first, concurrent) = tokio::join!(
        production.produce("demo", &first_trigger),
        production.produce("demo", &first_trigger),
    );
    let first = first.unwrap();
    assert_that!(&(concurrent.unwrap().id)).is_equal_to(first.id);

    let cross_trigger = production.produce("demo", &second_trigger).await.unwrap();
...
async fn new_project_gets_default_work_item_automation() {
    let (_temp, store) = test_store().await;
    let automations = crate::backend::automation::rules::tests::service(&store)
        .list("demo")
        .await
        .unwrap();
...
async fn trigger_create_and_update_round_trip_mutability() {
    ...
    let (_temp, store) = test_store().await;
    let default_personality = crate::backend::automation::personalities::tests::service(&store)
        .list("demo")
        .await
        .unwrap()[0]
        .clone();
...
async fn restoring_a_trigger_revision_appends_new_immutable_history() {
    ...
    let revisions = crate::backend::automation::rules::tests::service(&store)
        .revisions("demo", trigger.id)
        .await
        .unwrap();
    ...
    let original = revisions
        ...
        .unwrap();
    let restored = crate::backend::automation::rules::tests::service(&store)
        .restore("demo", trigger.id, original.id)
        .await
        .unwrap();
...
async fn trigger_rejects_cross_project_personality() {
    ...
    crate::backend::projects::tests::service(&store, crate::backend::events::UiEventBus::new())
        ...
        .await
        .unwrap();
    let other_default = crate::backend::automation::personalities::tests::service(&store)
        .list("other")
        .await
        .unwrap()[0]
        .id;
...
async fn default_selectors_route_labeled_items_to_refinement_automations() {
    ...
    let (_temp, store) = test_store().await;
    let refine_item = crate::backend::items::creation::tests::service(
        &store,
    ...
    .await
    .unwrap();
    let refine_item = crate::backend::items::labels::tests::service(&store, event_bus.clone())
        .add(
            "demo",
            refine_item.id,
            crate::shared::view_models::CreateWorkItemLabelRequest {
                key: "needs-refinement".to_owned(),
                value: None,
            },
            None,
            Default::default(),
        ...
        .await
        .unwrap();
    let verify_item = crate::backend::items::creation::tests::service(
        &store,
        crate::backend::events::UiEventBus::new(),
    )
    .create(
    ...
    .await
    .unwrap();
    let verify_item = crate::backend::items::labels::tests::service(&store, event_bus.clone())
        .add(
            "demo",
            verify_item.id,
            crate::shared::view_models::CreateWorkItemLabelRequest {
...
async fn work_item_automation_skips_stale_candidate_and_tries_next() {
    ...
    let second_trigger =
        crate::backend::automation::rules::tests::service_with_events(&store, event_bus.clone())
            ...
            .await
            .unwrap();
    let first_item = crate::backend::items::creation::tests::service(
        &store,
        crate::backend::events::UiEventBus::new(),
    )
    .create(
    ...
    .await
    .unwrap();
    let second_item = crate::backend::items::creation::tests::service(
        &store,
        crate::backend::events::UiEventBus::new(),
    )
    .create(
    ...
    .unwrap();
    crate::backend::items::claims::tests::service(&store, event_bus.clone())
        .claim_specific_item("demo", first_item.id, "agent-other")
        .await
        .unwrap()
        .unwrap();

    let outcome = crate::backend::automation::scheduling::tests::service(
        &store,
        std::sync::Arc::new(
            crate::backend::automation::launch::service::tests::service_with_sessions(&store, None),
        ),
        event_bus.clone(),
    )
    .run_candidates(
...
async fn exclusive_routing_uses_strict_priority_and_matches_diagnostics() {
    ...
    let explanation = crate::backend::automation::routing::tests::service(&store)
        .explain(
            "demo",
...
fn automation_view_for_score(
    ...
    AutomationTriggerView {
        ...
        personality_name: Some(
            crate::backend::automation::personalities::policy::DEFAULT_PERSONALITY_NAME.to_owned(),
        ),
        prompt: String::new(),
        work_item_selector: Some(default_work_item_selector()),
        priority,
...
async fn work_item_created_trigger_targets_new_item() {
    ...
    let outcomes = crate::backend::automation::scheduling::tests::service(
        &store,
        std::sync::Arc::new(
            crate::backend::automation::launch::service::tests::service_with_sessions(&store, None),
        ),
    ...
    let item =
        crate::backend::items::tests::service(&store, crate::backend::events::UiEventBus::new())
            ...
            .await
            .unwrap();
    let triggers = crate::backend::automation::rules::tests::service(&store)
        .list("demo")
        .await
        .unwrap();
    ...
    let trigger_runs = crate::backend::runs::queries::tests::service(&store)
        .list_for_trigger("demo", trigger.id, None)
        .await
        .unwrap();
...
async fn queued_work_producing_trigger_creates_item_without_agent_run() {
    ...
    let trigger = crate::backend::automation::rules::tests::service(&store)
        .list("demo")
        .await
        .unwrap()
        .into_iter()
...
async fn queued_evaluations_wait_until_project_is_active() {
    ...
    let trigger =
        crate::backend::automation::rules::tests::service_with_events(&store, event_bus.clone())
            ...
            .unwrap();
    crate::backend::automation::rules::tests::service_with_events(&store, event_bus.clone())
        .schedule("other", trigger.id)
        .await
        .unwrap();

    let active_project_ids = vec![ProjectRepository::new(store.db()).id("demo").await.unwrap()];
    let outcomes = crate::backend::automation::scheduling::tests::service(
        &store,
        std::sync::Arc::new(
            crate::backend::automation::launch::service::tests::service_with_sessions(&store, None),
    ...
    .run_due(Some(&active_project_ids), None)
    .await
    .unwrap();
    let trigger = crate::backend::automation::rules::tests::service(&store)
        .list("other")
...
async fn stale_scheduler_scope_does_not_activate_same_name_replacement() {
    ...
    let (temp, store) = test_store().await;
    let old_project_id = ProjectRepository::new(store.db()).id("demo").await.unwrap();
    ...
    .run_due(Some(&stale_project_ids), None)
    .await
    .unwrap();
    let trigger = crate::backend::automation::rules::tests::service(&store)
        .list("demo")
...
async fn rule_policy_validation_agrees_for_operator_bundle_and_storage_inputs() {
    ...
    use crate::backend::automation::bundles::policy as automation_bundles;
    use serde_json::json;

    let (_temp, store) = test_store().await;
    let original = crate::backend::automation::rules::tests::service(&store)
        .list("demo")
        .await
        .unwrap()
        .remove(0);
    ...
    for (effect, fields, expected) in cases {
        ...
        let error = crate::backend::automation::rules::tests::service_with_events(
            &store,
            event_bus.clone(),
        )
        ...
        .update_from_input("demo", original.id, input.clone())
        .await
        .unwrap_err();
        assert_that!(&error.to_string()).contains(expected);
    ...
    assert_that!(
        &crate::backend::automation::rules::tests::service(&store)
            .list("demo")
            .await
            .unwrap()
            .len()
...
async fn invalid_stored_policy_cannot_be_read_or_queued() {
    ...
    for timeout in [0, -1] {
        ...
        let error = crate::backend::automation::rules::tests::service(&store)
            .get("demo", &original.id.to_string())
            .await
            .unwrap_err();
...
pub(crate) fn service(store: &Store) -> std::sync::Arc<super::service::RuleService> {
    service_with_events(store, crate::backend::events::UiEventBus::new())
}
pub(crate) fn service_with_events(
    store: &Store,
    events: crate::backend::events::UiEventBus,
) -> std::sync::Arc<super::service::RuleService> {
    use std::sync::Arc;
    Arc::new(super::service::RuleService::new(
        Arc::new(crate::backend::storage::TransactionManager::new(store)),
        Arc::new(ProjectRepository::new(store.db())),
        Arc::new(super::repository::RuleRepository),
