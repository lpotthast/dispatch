...
pub(crate) struct RunWorkspaceRuntime;
...
impl RunWorkspaceRuntime {
    pub(crate) fn remove_run_workspace(
        ...
        if let Some(worktree_path) = worktree_path {
            self.prune_git_worktree(repo_path, branch_name, worktree_path)?;
        ...
        match repo.find_branch(branch_name, BranchType::Local) {
            ...
            Err(err) => {
                Err(err)
                    .context_with(|| format!("failed to find Dispatch branch '{branch_name}'"))?;
            }
        }
        Ok(())
    }
...
mod tests {
    ...
    fn deletion_cleanup_removes_dispatch_worktree_and_branch() {
        ...
        RunWorkspaceRuntime
            .remove_run_workspace(&repo_path, Some(&branch_name), Some(&worktree_path))
            .unwrap();
    ...
    fn deletion_cleanup_detaches_head_before_removing_dispatch_branch() {
        ...
        RunWorkspaceRuntime
            .remove_run_workspace(temp.path(), Some(&branch_name), None)
            .unwrap();
