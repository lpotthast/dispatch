...
impl WorkspaceRuntime {
    pub(crate) async fn choose_folder(&self) -> Result<Option<String>> {
        choose_folder_path(&self.config.os).await
    }

    pub(crate) fn from_env() -> Self {
        Self {
            config: WorkspaceOpenConfig::from_env(),
        }
    }
    pub(crate) async fn open(
        ...
        self.config.command_for(target, &path)?.run().await
    }
    pub(crate) fn available_editors(&self) -> Vec<WorkspaceEditorView> {
        self.config.available_editors()
    }
}
...
async fn choose_folder_path(os: &str) -> Result<Option<String>> {
    match os {
