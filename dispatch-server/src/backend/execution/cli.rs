...
impl CliConfig {
    pub(crate) fn from_env() -> Self {
        Self {
            development: env::var_os(DEVELOPMENT_ENV),
            path: env::var_os("PATH"),
            cargo: env::var_os("CARGO").unwrap_or_else(|| OsString::from("cargo")),
            target_dir: dispatch_cli_target_dir(),
        }
    }
}
...
impl CliService {
    ...
    pub(crate) async fn resolve(&self) -> Result<PathBuf> {
        if parse_development_mode(self.config.development.as_deref())? {
            return build_dev_dispatch_cli(
                &development_cli_sources()?,
                self.config.target_dir.clone(),
                self.config.cargo.clone(),
            )
            .await;
        }
        let path = installed_dispatch_cli_path(self.config.path.as_deref())?;
...
fn installed_dispatch_cli_path(path: Option<&OsStr>) -> Result<PathBuf> {
    let path = path
...
mod tests {
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn cli_services_resolve_only_their_injected_configuration() {
        let first = TempDir::new().unwrap();
        let second = TempDir::new().unwrap();
        for dir in [&first, &second] {
            let cli = dir.path().join("dispatch");
            fs::write(
                &cli,
                "#!/bin/sh\nprintf 'Dispatch agent-facing API relay\\n'\n",
            )
        ...
        let service = |dir: &TempDir| {
            CliService::new(CliConfig {
                development: None,
                path: Some(dir.path().as_os_str().to_owned()),
                cargo: "unused".into(),
                target_dir: dir.path().join("unused"),
            })
        };
        assert_that!(&service(&first).resolve().await.unwrap())
            .is_equal_to(first.path().join("dispatch").canonicalize().unwrap());
        assert_that!(&service(&second).resolve().await.unwrap())
            .is_equal_to(second.path().join("dispatch").canonicalize().unwrap());
    }
}
