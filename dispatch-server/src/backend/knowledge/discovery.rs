...
impl Discovery {
    ...
    fn walk(&mut self, dir: &Path, inherited: &Rules) {
        self.walk_scope(dir, inherited, false);
    }

    pub(crate) fn sources(workspace: &Path) -> Self {
        let mut output = Self::default();
        output.walk_scope(workspace, &Rules::default(), true);
        output.files.sort();
        output.controls.sort();
        output
    }

    fn walk_scope(&mut self, dir: &Path, inherited: &Rules, sources: bool) {
        ...
        for entry in entries {
            ...
            if kind.is_dir() {
                self.walk_scope(&path, &rules, sources);
            } else if kind.is_file() && (sources || path.extension().is_some_and(|ext| ext == "md"))
            {
