...
pub(crate) fn install() {
    ...
    *EVENT_BUS
        .write()
        .expect("Dispatch UI event bus lock is poisoned") = Some(Arc::new(bus));
}
...
fn publish(build: impl FnOnce(u64, String) -> UiEvent) {
    let Some(bus) = EVENT_BUS
        .read()
        .expect("Dispatch UI event bus lock is poisoned")
        .clone()
    else {
        return;
    };
...
pub(crate) fn publish_project_deleted(project_id: i64, project: &str) {
    let project = project.to_owned();
    publish(|sequence, timestamp| UiEvent::ProjectDeleted {
        sequence,
        timestamp,
        project_id,
        project,
...
fn event_bus() -> Arc<UiEventBus> {
    let Some(bus) = EVENT_BUS
        ...
        return EVENT_BUS
            ...
            .expect("Dispatch UI event bus lock is poisoned")
            .as_ref()
            .expect("Dispatch UI event bus was just installed")
            .clone();
    };
    bus
}
