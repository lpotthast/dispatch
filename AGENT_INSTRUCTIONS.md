...
- Before adding a knowledge section or document, read the relevant routing summaries and search for existing owners of the concept. Extend the owning explanation; do not append detail to the document you happened to be reading. Follow the project's actual document ownership and paths rather than assuming fixed filenames.
- Each durable explanation has one owner. Put UI layout and controls in the relevant UI owner and lifecycle rules in their workflow owner. State the detailed contract once, including within that owner. Elsewhere keep only the consequence needed at that scope and a direct Markdown link to the owner or section. A link or frontmatter relationship does not make repeated detailed rules acceptable.
...
- Before finishing knowledge edits, compare changed sections with existing owners, related explanations, and affected summaries for duplicated contracts, misplaced detail, and missing owner links. Consolidate duplication without losing unique requirements, exceptions, or rationale, and repair affected routes. Structural checks do not replace this semantic review.
